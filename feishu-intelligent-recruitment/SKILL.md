...
1. **获取源信息 (Source Acquisition & File Fallback)**：
   - 【简历定位规则】：当接收到简历处理任务或 Bitable 记录时，**大主管必须去读取并查看「解析简历链接 (MD)」和「原始简历链接 (PDF)」的内容，看看哪个可以作为有效简历使用**。
   - 只要找到一个包含真实简历内容的（优先看 MD，不行再看 PDF），就直接用它跑一次流程。
   - 如果两个文件都没有内容或者都不存在，立刻停下来向主人要。
