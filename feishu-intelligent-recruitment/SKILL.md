...
1. **获取源信息 (Source Acquisition & File Fallback)**：
   - 【简历定位规则】：当接收到简历处理任务或 Bitable 记录时，**大主管禁止自行进行复杂的 Record ID 检索或盲猜**。必须遵循飞书特定的极简回退机制 (KISS原则)：
     1. 首先检查表格中的【解析简历链接 (MD)】，如果有，直接用它跑一次流程。
     2. 如果没有 MD，再检查【原始简历链接 (PDF)】，如果有，直接用它跑一次流程。
     3. 如果都没有，立刻停下来向主人要。
     - **严禁**自行对比两个文件哪个更完整，越直接越好。
