...
1. **获取源信息 (Source Acquisition & File Fallback)**：
   - 【简历定位规则】：当接收到简历处理任务或 Bitable 记录时，**大主管禁止自行进行复杂的 Record ID 检索或盲猜**。必须遵循飞书特定的极简内容探测机制：
     - **去查看「解析简历链接 (MD)」和「原始简历链接 (PDF)」**，分别提取并**看哪个是有实际内容的完整简历**。因为在飞书特定的场景下，可能两者都存在，但其中一个只是空壳或短摘要。**谁有真实内容，就直接用谁跑一次流程**。
     - 如果都没有内容，立刻停下来向主人要，严禁死循环搜索。
